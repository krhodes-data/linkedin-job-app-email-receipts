        ...
        if message.is_multipart():
            for part in message.walk():
                ...
                if ctype == 'text/plain':
                    ...
                    print(f"Body is of type {ctype}")
                    break
        else:
            body = message.get_payload(decode=True)
        body = body.decode('utf-8')
        hyperlinks = re.findall('https.+?\\r\\n',body)
        if message.get("Subject").startswith("Kevin, your application was sent to "):
            print("Email found - adding to spreadsheet")
            emailSubject = message.get("Subject")
            dateApplied = message.get("Date")
            companyAppliedTo = emailSubject.split("to ")[1]
            positionAppliedFor = body.split('\n')[2]
            ...
            print(f"Deleting \"{emailSubject}\"and waiting 2 seconds to parse next email...")
